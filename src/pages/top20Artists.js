  ...
  return (
    <div className="bg-black min-h-screen">
      <div className="max-w-md mx-auto relative pb-24">
        ...
        {top20.map((item, idx) => (
          <li key={idx} className="relative bg-linear-to-r from-[#212121] from-50% to-[#121212] to-100% pl-4 p-3 py-4 rounded-[10px] mb-5">
            <div className="flex gap-3 space-between relative">
              <p className="py-1">#{idx + 1}</p>
              <span className="py-1">{item.nome}</span>
              <span className="absolute py-1 text-[#B3B3B3] text-[12px] right-5">{Math.floor(item.msTotal / 60000)} min ouvidos</span>
            </div>
          </li>
        ))}
      </ol>
